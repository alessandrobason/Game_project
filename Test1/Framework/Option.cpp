...
OptionMenu::OptionMenu(sf::RenderWindow* window, InputHandler* input, RoomManager* rm) : Room("MainScreen", window, input, rm), GUImanager(window, input, rm) {
	...
	texturestoload.push_back({ "slider", "GUI/slider_outline.png" });
	...
	//fullscreenhoralign.setAlign(GUIelement::ALIGN::CENTER, GUIelement::ALIGN::CENTER);
	...
	//mutehoralign.setAlign(GUIelement::ALIGN::CENTER, GUIelement::ALIGN::TOP);
	mutehoralign.load();

	GUItext mutelabel;
	mutelabel.setParent(&mutehoralign);
	mutelabel.setFont(&roommanager->fonts["font"]);
	mutelabel.setText("Mute music");
	...
	volume.setTileSize(4);
	volume.setColor(sf::Color::Blue, sf::Color::Red);
